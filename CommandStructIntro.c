#include "vex.h"

using namespace vex;


#define KP 0.5
#define GYRO_SPEED 50
#define BUFFER 20
#define MOVEMENT_INCREASE_FORWARD_P 3


#define C  1
#define D  2
#define E  3
#define F  4
#define G  5
#define A  6
#define B  12


vex::brain Brain;

// Motors
vex::motor rightArmMotor(PORT9, true);
vex::motor leftArmMotor(PORT3, false);

vex::motor RightFrontChassisMotor(PORT8, false);
vex::motor RightBackChassisMotor(PORT1, true);
vex::motor LeftFrontChassisMotor(PORT2, true);
vex::motor LeftBackChassisMotor(PORT7, false);

// Pneumatics
vex::pneumatic armSolenoid(PORT10);

// Sensors
vex::gyro firstStageArmGyro(PORT5);
vex::bumper backTouchSensor(PORT12);

// Controller
vex::controller Controller;

// Timer
vex::timer Timer;

//Gyro
vex::inertial Gyro;

vex::touchled touchLED(PORT4);

//vex::aivision AiVision(PORT2, aivision::ALL_AIOBJS);
vex::optical OpticalSensor(PORT11);
    

//*!!Code automatically generated by 'ROBOTC' configuration wizard               !!*//

#define MAX_COMMAND_LENGTH 50
#define RADIUS_OF_ROBOT 80
#define RADIUS_OF_WHEEL 21
#include "Headers/MyHeaders.h"


void createCommands()
{
	bot.index = 0;

	for(int i = 0; i < 4; i++)
	{
		initMoveByDegreeData(distanceToDegrees(100), distanceToDegrees(100), Acc, Acc);

			initMoveByDegreeData(100,100,Acc,Acc, true);
			initMoveByDegreeData(-100,-100,Acc,Acc, true);
			initMoveByDegreeData(100,100,Acc,Acc, true);
			initMoveByDegreeData(-100,-100,Acc,Acc, true);
			initStopRecData();

		initMoveByDegreeData(degreesToMove(90), -degreesToMove(90), Acc, Acc);

			initMoveByDegreeData(100,100,Acc,Acc, true);
			initMoveByDegreeData(-100,-100,Acc,Acc, true);
			initMoveByDegreeData(100,100,Acc,Acc, true);
			initMoveByDegreeData(-100,-100,Acc,Acc, true);
			initStopRecData();
	}

		//initMoveByDegreeData(distanceToDegrees(250), distanceToDegrees(450), Acc, Acc);



	initStopData();

	bot.index = 0;
}
void runCommands(Bot bot)
{
	BaseCommand& command = bot.commands[bot.index];
	switch(command.commandType)
	{
		case AttachMotorRec:
		{
			MoveAttachMotorData data;

			memcpy(&data, command, sizeof(MoveAttachMotorData));

			configure(data);

			if(!hasRan(data)) return;

			cleanup(data);

			finalize();


		}
		break;

		case MoveByDegreesRec:
		{
			RobotDrivetrainData data;

			memcpy(&data, command, sizeof(RobotDrivetrainData));

			configure(data);



			if(!hasRan(data)) return;

			cleanup(data);

			finalize();




		}
		break;

		/*
***********************************************		Your commands are being created while finalize is running *********************************************************************
		*/

		case LineFollowRec:
		{
			LineFollowData data;

			memcpy(&data, command, sizeof(LineFollowData));

			configure(data);

			if(!hasRan(data)) return;

			cleanup(&(LineFollowData)command);

			finalize();


		}
		break;

		case LineFollow:
		{
			LineFollowData data;

			memcpy(&data, command, sizeof(LineFollowData));

			configure(data);

			if(!hasRan(data)) return;

			cleanup(&(LineFollowData)command);

			finalize();

		}
		break;

		case AttachMotor:
		{
			MoveAttachMotorData data;

			memcpy(&data, command, sizeof(MoveAttachMotorData));

			configure(data);

			if(!hasRan(data)) return;

			cleanup(data);

			finalize();


		}
		break;

		case MoveByDegrees:
		{
			if(initiateStall(bot.left)||
			initiateStall(bot.right)||
			initiateStall(bot.center))
			{
				bot.isRecovery = true;
				writeDebugStreamLine("Recovery");
			}

			RobotDrivetrainData data;

			memcpy(&data, command, sizeof(RobotDrivetrainData));

			configure(data);

			if(!hasRan(data)) return;

			cleanup(data);

			finalize();
		}
		break;

		case StopRecovery:
		{
			bot.isRecovery = false;
		}
		break;

		case Stop:
		{
			bot.left.motorPort.stop();
			bot.right.motorPort.stop();
			bot.center.motorPort.stop();
		}
		break;
	}

}
void RecoveryChecker(bool& isCheckForRecovery, int startindex, int endindex)
{
	if(bot.index >= startindex && bot.index <= endindex)
	{
		isCheckForRecovery = true;
	}
	else
	{
		isCheckForRecovery = false;
	}
}


task main()
{
	vexMotorGroup left;
	vexMotorGroup right;
	vexMotorGroup center;
	left.firstMotor = LeftFrontChassisMotor;
	left.secondMotor = LeftBackChassisMotor;
	right.firstMotor = RightFrontChassisMotor;
	right.secondMotor = RightBackChassisMotor;
	center.firstMotor = rightArmMotor;
	center.secondMotor = leftArmMotor;
	bot.left.profile.previousPosition = 0;
	bot.right.profile.previousPosition = 0;
	bot.center.profile.previousPosition = 0;
	bot.left.motorPort = left;
	bot.right.motorPort = right;
	bot.center.motorPort = center;

	
	
	bot.isCheckForRecovery = false;
	bot.isRecovery = false;
	clearDebugStream();
	createCommands();

	while(true)
	{


		trackVelocity(bot.left);
		trackVelocity(bot.right);


		//if(SensorValue[sonar] < 24 && !bot.isRecovery && bot.isCheckForRecovery)
		//{
		//	bot.index++;
		//	//bot.isRecovery = true;
		//}

		RecoveryChecker(bot.isCheckForRecovery, 0, 40);
		displayTextLine(6, "Bot: %i", bot.index);
		runCommands(bot);
		displayTextLine(2, "IsRecovery: %i", bot.isRecovery);


		realTime();
	}


}
